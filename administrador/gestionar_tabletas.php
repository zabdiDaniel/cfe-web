...
if (isset($_POST['delete_activo'])) {
    try {
        ...
        $stmt->close();
        header("Location: " . BASE_URL . "administrador/gestionar_tabletas.php?page=$page" . (!empty($search) ? "&search=" . urlencode($search) : ""));
        exit();
    } catch (Exception $e) {
        ...
        <table class="table table-bordered" id="tabletasTable">
            ...
            <tbody>
                <?php if (empty($tabletas)): ?>
                    <tr><td colspan="<?= count($columns) + 1 ?>" style="text-align: center;">No hay tabletas disponibles.</td></tr>
                <?php else: ?>
                ...
                <form id="updateForm" method="POST" action="<?= BASE_URL ?>administrador/update_tablet.php">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" id="editMarca" name="marca" required style="border-radius: 8px;">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="editModelo" class="form-label">Modelo</label>
                            <input type="text" class="form-control" id="editModelo" name="modelo" required style="border-radius: 8px;">
                        </div>
                        <div class="col-md-6 mb-3">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" id="editUbicacionRegistro" name="ubicacion_registro" style="border-radius: 8px;">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="editFechaRegistro" class="form-label">Fecha Registro</label>
                            <input type="datetime-local" class="form-control" id="editFechaRegistro" name="fecha_registro" style="border-radius: 8px;">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="editMarcaChip" class="form-label">Marca Chip</label>
                            <input type="text" class="form-control" id="editMarcaChip" name="marca_chip" style="border-radius: 8px;">
                        </div>
                        <div class="col-md-6 mb-3">
...
document.addEventListener('click', function(event) {
    ...
    console.log('Cargando datos para edición:', { activo: row.dataset.activo });
    ...
    <?php echo json_encode(array_column($columns, 'id')); ?>.forEach(colId => {
        ...
        if (input) {
            ...
            console.log(`Asignado ${colId}: value=${input.value}`);
        } else {
            console.warn(`Input no encontrado para ${colId}: ${inputId}`);
        }
    });
});
...
document.getElementById('updateForm').addEventListener('submit', function(event) {
    ...
    .then(data => {
        window.location.href = '<?= BASE_URL ?>administrador/gestionar_tabletas.php?success=Tableta actualizada correctamente';
    })
    .catch(error => {
        console.error('Error:', error);
        window.location.href = '<?= BASE_URL ?>administrador/gestionar_tabletas.php?error=Error al actualizar la tableta';
    });
});
</script>