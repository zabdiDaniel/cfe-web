...
// Contar el total de trabajadores
...
if (isset($_POST['delete_rpe'])) {
    ...
    $stmt->execute();
    header("Location: " . BASE_URL . "administrador/personal.php?page=$page" . (!empty($search) ? "&search=" . urlencode($search) : ""));
    exit();
}
...
$columns = [
    ...
    ['id' => 'estatus', 'label' => 'Estatus', 'field' => 'estatus'],
    ['id' => 'fecha_registro', 'label' => 'Fecha Registro', 'field' => 'fechaRegistro'],
    ['id' => 'centro_trabajo', 'label' => 'Centro Trabajo', 'field' => 'centroTrabajo'],
    ['id' => 'seccion_sindical', 'label' => 'Sección Sindical', 'field' => 'seccionSindical'],
...
<div class="container mt-5">
    ...
    <div class="mb-4 d-flex align-items-center gap-3">
        <form method="GET" action="<?= BASE_URL ?>administrador/personal.php" class="d-flex" style="max-width: 400px; flex-grow: 1;">
        ...
        <table class="table table-bordered" id="trabajadoresTable">
            ...
            <tbody>
                <?php if (empty($trabajadores)): ?>
                    <tr>
                        <td colspan="<?= count($columns) + 1 ?>" style="text-align: center;">No hay trabajadores disponibles.</td>
                    </tr>
                <?php else: ?>
                        ...
                        <tr data-rpe="<?= htmlspecialchars($trabajador['rpe']) ?>" class="<?= $index % 2 === 0 ? 'row-dark' : 'row-light' ?>">
                            <?php foreach ($columns as $col): ?>
                                <td data-col-id="<?= $col['id'] ?>">
                                    ...
                                    <?php elseif ($col['id'] === 'estatus'): ?>
                                        <?= $trabajador['estatus'] ? 'Activo' : 'Inactivo' ?>
                                    <?php else: ?>
                                        <?= htmlspecialchars($trabajador[$col['field']] ?? 'N/A') ?>
                                    <?php endif; ?>
                                </td>
                            <?php endforeach; ?>
            ...
            <div class="modal-body">
                <form id="updateForm" method="POST" action="<?= BASE_URL ?>administrador/update_trabajador.php">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control" id="editEstatus" name="estatus" required style="border-radius: 8px;">
...
<script>
    ...
    function adjustTableWidth() {
        ...
        const visibleColumns = document.querySelectorAll('#trabajadoresTable th:not(.hidden)');
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.querySelectorAll('.btn-edit').forEach(button => {
            button.addEventListener('click', function() {
                ...
                const estatusCell = row.querySelector('td[data-col-id="estatus"]');
                const fechaRegistroCell = row.querySelector('td[data-col-id="fecha_registro"]');
                const centroTrabajoCell = row.querySelector('td[data-col-id="centro_trabajo"]');
                const seccionSindicalCell = row.querySelector('td[data-col-id="seccion_sindical"]');
                const categoriaCell = row.querySelector('td[data-col-id="categoria"]');
                ...
                document.getElementById('editEstatus').value = estatusCell ? (estatusCell.textContent.trim() === 'Activo' ? '1' : '0') : '0';
                document.getElementById('editFechaRegistro').value = fechaRegistroCell ? fechaRegistroCell.textContent.trim() : 'N/A';
                document.getElementById('editCentroTrabajo').value = centroTrabajoCell && centroTrabajoCell.textContent !== 'N/A' ? centroTrabajoCell.textContent.trim() : '';
                document.getElementById('editSeccionSindical').value = seccionSindicalCell && seccionSindicalCell.textContent !== 'N/A' ? seccionSindicalCell.textContent.trim() : '';
                document.getElementById('editCategoria').value = categoriaCell && categoriaCell.textContent !== 'N/A' ? categoriaCell.textContent.trim() : '';
                document.getElementById('editFoto').src = fotoCell ? fotoCell.src : '<?= BASE_URL ?>cfe-api/uploads/perfiles/default.jpg';
            });
        });

        // Inicializar columnas
        const savedColumns = JSON.parse(localStorage.getItem('trabajadoresColumns')) || {};
        ...
        // Orden de columnas
        const savedOrder = JSON.parse(localStorage.getItem('trabajadoresColumnOrder')) || [];
        if (savedOrder.length === <?php echo count($columns); ?>) {
            const thead = document.querySelector('#trabajadoresTable thead tr');
            const tbodyRows = document.querySelectorAll('#trabajadoresTable tbody tr');
    ...
    document.getElementById('applyColumns').addEventListener('click', () => {
        ...
        localStorage.setItem('trabajadoresColumns', JSON.stringify(visibleColumns));
        adjustTableWidth();
    ...
    document.getElementById('resetColumns').addEventListener('click', () => {
        localStorage.removeItem('trabajadoresColumns');
        document.querySelectorAll('#columnSelectForm .form-check-input').forEach(checkbox => {
        ...
        localStorage.setItem('trabajadoresColumns', JSON.stringify({}));
        adjustTableWidth();
        bootstrap.Modal.getInstance(document.getElementById('columnModal')).hide();
    });
    ...
    if (orderList) {
        ...
        document.getElementById('applyOrder').addEventListener('click', () => {
            ...
            localStorage.setItem('trabajadoresColumnOrder', JSON.stringify(newOrder));
    ...
    document.getElementById('updateForm').addEventListener('submit', function(event) {
        ...
        fetch('<?= BASE_URL ?>administrador/update_trabajador.php', {
                ...
                if (data.success) {
                    ...
                    if (row) {
                        const cells = {
                            ...
                            'estatus': row.querySelector('td[data-col-id="estatus"]'),
                            'centro_trabajo': row.querySelector('td[data-col-id="centro_trabajo"]'),
                            'seccion_sindical': row.querySelector('td[data-col-id="seccion_sindical"]'),
                            'categoria': row.querySelector('td[data-col-id="categoria"]')
                        };
                        ...
                        if (cells.estatus) cells.estatus.textContent = data.data.estatus === '1' ? 'Activo' : 'Inactivo';
                        if (cells.centro_trabajo) cells.centro_trabajo.textContent = data.data.centroTrabajo ?? 'N/A';
                        if (cells.seccion_sindical) cells.seccion_sindical.textContent = data.data.seccionSindical ?? 'N/A';
                        if (cells.categoria) cells.categoria.textContent = data.data.categoria ?? 'N/A';
            ...
            .catch(error => {
                ...
                showToast('Error', 'No se pudo actualizar el trabajador', false);
            });
    });
</script>
