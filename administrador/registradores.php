...
// Contar el total de registradores (con búsqueda si aplica)
...
// Obtener registradores para la página actual (con búsqueda si aplica)
...
if (isset($_POST['delete_rpe'])) {
    ...
    $stmt->execute();
    header("Location: " . BASE_URL . "administrador/registradores.php?page=$page" . (!empty($search) ? "&search=$search" : ""));
    exit();
}
?>
...
<div class="container mt-5">
    ...
    <!-- Barra de búsqueda -->
    <div class="mb-4">
        <form method="GET" action="<?= BASE_URL ?>administrador/registradores.php" class="d-flex" style="max-width: 400px;">
        ...
        <table class="table table-bordered" id="registradoresTable">
            ...
            <tbody>
                <?php if (empty($registradores)): ?>
                    <tr><td colspan="7" style="text-align: center;">No hay registradores disponibles.</td></tr>
                <?php else: ?>
                    ...
                    <tr data-rpe="<?= htmlspecialchars($registrador['rpe']) ?>" class="<?= $index % 2 === 0 ? 'row-dark' : 'row-light' ?>">
                        <td><?= htmlspecialchars($registrador['rpe']) ?></td>
                        <td><?= htmlspecialchars($registrador['nombre']) ?></td>
                        <td><?= htmlspecialchars($registrador['correo'] ?? 'N/A') ?></td>
                        <td><?= htmlspecialchars($registrador['tipo_usuario']) ?></td>
                        <td><?= htmlspecialchars($registrador['fecha_registro'] ?? 'N/A') ?></td>
                        <td>
...
<script>
    // Editar registrador (llenar el modal)
    document.querySelectorAll('.btn-edit').forEach(button => {
        button.addEventListener('click', function() {
            ...
            document.getElementById('editCorreo').value = row.cells[2].textContent === 'N/A' ? '' : row.cells[2].textContent;
            document.getElementById('editTipoUsuario').value = row.cells[3].textContent;
        });
    });
